...
public class DownloadController {
	...
	@RequiresRoles("admin")
	@ResponseBody
	@GetMapping("/exportaddress/{act_id}")
	public void exportAllAddressByActId(@PathVariable("act_id")Integer act_id, HttpServletRequest request, HttpServletResponse response) {
		response.reset(); //清除buffer缓存
		Map<String,Object> map = new HashMap<String,Object>();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddhhmmss");
		response.setHeader("Content-Disposition", "attachment;filename="+sdf.format(new Date())+".xlsx");
		...
		XSSFWorkbook workbook=null;
		try {
			workbook = downloadService.exportAllAddressByActid(act_id);
			OutputStream out = response.getOutputStream();
			BufferedOutputStream bout = new BufferedOutputStream(out);
			bout.flush();
			workbook.write(bout);
